...
ldelete(int n, bool kflag) {
  ...
  while (n != 0) {
    ...
    if (chunk == 0) {              /* End of line, merge.  */
      lchange(WINSCR::WFHARD);
      if (delnewline() == false || (kflag && !kinsert('\n'))) {
        return false;
      }
      --n;
      continue;
 ...
 * require  that  lines be moved about in memory.  Return flase on
 ...
 * buffer.  Handcraft the EOL on the end.  Return true if it worked
 * and false if you ran out of room.
 */

bool
addline(BUFFER* bp, const EMCHAR* text) {
