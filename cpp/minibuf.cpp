#if !defined(lint)
static  char rcsid[] = "$Id: minibuf.cpp,v 1.20 2018/09/09 07:21:10 jullien Exp $";
#endif
...
mledit(const EMCHAR* prompt, EMCHAR* buf, int nbuf) {
 ...
 loop:
  cpos       = 0;
  completion = COMPLETION::COMPLETE_ONE;
  ...
  for (;;) {
    ...
    default:
      ...
      if (((c == ' ') || (c == '\t')) && (complete.flag != 0L)) {
        buf[cpos] = '\000';
        completion  = COMPLETION::COMPLETE_ONE;
        if ((s = (*complete.fn)(prompt,buf)) != nullptr) {
          (void)emstrcpy(buf, s);
          if (completion == COMPLETION::COMPLETE_AGAIN) {
            goto loop;
          }
          ...
          complete.flag = 0L;
          if (completion == COMPLETION::COMPLETE_ABORT) {
            return ABORT;
          }
          completion = COMPLETION::COMPLETE_FAIL;
